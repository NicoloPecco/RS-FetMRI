...
% ------------------------
z_thresh = 1.5;             %global signal threshold
...
stats_file='';
output_dir='';
%Passo i parametri del for loop (i e session file)
i = evalin('base','i');
Session_name= evalin('base','Session_name');
out_ppt=evalin('base','folder_out');
output_path=[out_ppt,'/',Session_name{i,1}];
...
%     use_diff_motion=0;use_diff_global=0;use_norms=1;
        ...
        for i = 1:nscans,
            ...
            if numel(maskscan)>=100, % stop maskcan from growing too much
                ...
                try
                    ...
                    fprintf('warning: unable to write to ',output_dir,' folder. Writing output files to ',pwd,' instead.');
                    output_dir=pwd;
                    save(fullfile(output_dir,[art_mask_temporalfile(1:end-4),num2str(maskscan_files),'.mat']),'maskscan');
                end
                maskscan={};
...
nome_path_ptovv=[path_general,'M4_BS_02_Scrub/art_single_sessions/'];
%nome_path_ptovv='/data/00_rsFetMRI_PreProc_1.5T/Dati_fmri/M4_BS_02_Scrub/art_single_sessions/';
%save outliers
out_idx = round(str2num(get(handles.all_outliers, 'String')));
filename='art_outliers.mat';
filename = strcat(char(nome_path_ptovv), char(filename));
save(filename,'out_idx', '-mat');
%save motion_statistics
mv_stats = getappdata(handles.mvthresh,'mv_stats');
filename='art_motion_statistics.mat';
filename = strcat(char(nome_path_ptovv), char(filename));
save(filename ,'mv_stats','-mat');         
% save figure      
filename='art_figure.jpg';
filename = strcat(char(nome_path_ptovv), char(filename));
saveas(gcf,filename);
catch
    disp(':Error encountered during ART file creation. Skipping this step');
end
...
for j=1:num_sess,
    ...
    try
        R=R1;save(fullfile(datafiles_path,['art_regression_outliers.mat']),'R','-mat'); %#ok<NASGU>
        R=R2;save(fullfile(datafiles_path,['art_regression_outliers_and_movement.mat']),'R','-mat'); %#ok<NASGU>
...
use_norms=1;
SPMfile=[];
% Richiamo nuovamente le variabili i e Session_name
i = evalin('base','i');
Session_name= evalin('base','Session_name');
out_ppt=evalin('base','folder_out');
% Indico dove trovare l'immagine mean, l'outputpath;
%mask_file=['/home/dati/Nico_auto_15T/Dati_fmri/06_Scrub_Whithin_Session/',Session_name{i,1},'/','mean',Session_name{i,1},'_0001.nii'];
output_path=[out_ppt,'/',Session_name{i,1}];
output_dir=output_path;
